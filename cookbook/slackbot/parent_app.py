...
PARENT_APP_STATE_BLOCK_NAME = "scalify-parent-app-state"
PARENT_APP_STATE = JSONBlockState(block_name=PARENT_APP_STATE_BLOCK_NAME)

EVENT_NAMES = [
    "scalify.assistants.SubAssistantRunCompleted",
]
...
def take_lesson_from_interaction(
    ...
    """You are an expert {{ observer_role }}, and you are counseling an AI assistant named Scalify.

    {{ irrelevant_topics }} is not relevant to Scalify's purpose, and has 0 relevance.

    Here is the transcript of an interaction between Scalify and a user:
    {{ transcript }}
...
def excerpt_from_event(event: Event) -> str:
    ...
    return (
        ...
        f"\n\nScalify (the assistant) responded with: {ai_response}"
    )
...
parent_assistant_options = dict(
    instructions=(
        "Your job is learn from the interactions of data engineers (users) and Scalify"
        ...
        " provided to Scalify when interacting with users. Notes should be stored for"
        ...
        " 3 notes per user, but you may curate/update these over time for Scalify's"
...
async def lifespan(app: ReadyAPI):
    with Application(name="Scalify", **parent_assistant_options) as scalify:
        logger.debug_kv("👂 Listening for", " | ".join(EVENT_NAMES), "green")

        app.state.scalify = scalify
        task = asyncio.create_task(learn_from_child_interactions(scalify, EVENT_NAMES))
        yield
        task.cancel()
        try:
            await task
...
def emit_assistant_completed_event(
    ...
    event = emit_event(
        event="scalify.assistants.SubAssistantRunCompleted",
        resource={
