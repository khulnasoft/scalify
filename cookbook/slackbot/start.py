...
from scalify.tools.github import search_github_issues
from scalify.utilities.logging import get_logger
from scalify.utilities.slack import (
    SlackPayload,
    get_channel_name,
    get_workspace_info,
    post_slack_message,
...
BOT_MENTION = r"<@(\w+)>"
CACHE = JSONBlockState(block_name="scalify-thread-cache")
...
async def handle_message(payload: SlackPayload) -> Completed:
    ...
    if (user := re.search(BOT_MENTION, user_message)) and user.group(
        ...
        with Assistant(
            ...
            instructions=(
                "You are Scalify, the paranoid android from Hitchhiker's Guide to the"
...
app = ReadyAPI(lifespan=lifespan if ENABLE_PARENT_APP else None)


def get_parent_app() -> Application:
    scalify = getattr(app.state, "scalify", None)
    if not scalify:
        logger.warning("Scalify instance not available")
    return scalify
