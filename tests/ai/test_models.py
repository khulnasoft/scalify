...
class TestModels:
    ...
    def test_literal(self):
        ...
        class LLMConference(BaseModel):
            speakers: list[
                Literal["Adam", "Nate", "Jeremiah", "Scalify", "Billy Bob Thornton"]
            ]

        x = LLMConference(
            """
            The conference for best LLM framework will feature talks by
            Adam, Nate, Jeremiah, Scalify, and Billy Bob Thornton.
        """
        )
        assert len(set(x.speakers)) == 5
        assert set(x.speakers) == set(
            ["Adam", "Nate", "Jeremiah", "Scalify", "Billy Bob Thornton"]
        )
...
class TestInstructions:
    def test_instructions(self):
        ...
        @scalify.model(instructions="first translate the text to French")
        class Text(BaseModel):
            text: str
...
class TestAsync:
    async def test_basic_async(self):
        from scalify.ai.text import Model
