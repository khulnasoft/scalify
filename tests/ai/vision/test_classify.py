...
class TestVisionClassify:
    def test_ny_image_input(self):
        ...
        result = scalify.classify(data=img, labels=["urban", "rural"])
        assert result == "urban"
    ...
    def test_classify_wet_dog(self):
        ...
        animal = scalify.classify(img, labels=["dog", "cat", "bird", "fish", "deer"])
        assert animal == "dog"
...
class TestAsync:
    async def test_ny(self):
        ...
        result = await scalify.classify_async(img, labels=["urban", "rural"])
        assert result == "urban"
...
class TestMapping:
    def test_map(self):
        ...
        result = scalify.classify.map([ny, dc], labels=["urban", "rural"])
    ...
    async def test_map_async(self):
        ...
        result = await scalify.classify_async.map([ny, dc], labels=["urban", "rural"])
        assert isinstance(result, list)
        assert result == ["urban", "urban"]
