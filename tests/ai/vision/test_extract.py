...
class TestVisionExtract:
    def test_ny_image_input(self):
        ...
        locations = scalify.extract(data=img, target=Location)
    ...
    def test_two_cities(self):
        ...
        locations = scalify.extract([img_ny, img_dc], target=Location)
    ...
    def test_dog(self):
        ...
        result = scalify.extract(img, target=Animal)
        assert set(result) == {
...
class TestAsync:
    async def test_ny(self):
        ...
        locations = await scalify.extract_async(img, target=Location)
        assert len(locations) == 1
        assert_locations_equal(locations[0], Location(city="New York", state="NY"))
...
class TestMapping:
    def test_map(self):
        ...
        locations = scalify.extract.map([ny, dc], target=Location)
        assert len(locations) == 2
        assert_locations_equal(locations[0], Location(city="New York", state="NY"))
        assert_locations_equal(locations[1], Location(city="Washington", state="DC"))
    ...
    async def test_async_map(self):
        ...
        locations = await scalify.extract_async.map([ny, dc], target=Location)
        assert len(locations) == 2
        assert_locations_equal(locations[0], Location(city="New York", state="NY"))
        assert_locations_equal(locations[1], Location(city="Washington", state="DC"))
