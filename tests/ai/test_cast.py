...
class TestCast:
    class TestBuiltins:
        def test_cast_text_to_int(self):
            result = scalify.cast("one", int)
            assert result == 1

        def test_cast_text_to_list_of_ints(self):
            result = scalify.cast("one, TWO, three", list[int])
            assert result == [1, 2, 3]

        def test_cast_text_to_list_of_ints_2(self):
            result = scalify.cast("4 and 5 then 6", list[int])
            assert result == [4, 5, 6]

        def test_cast_text_to_list_of_floats(self):
            result = scalify.cast("1.0, 2.0, 3.0", list[float])
            assert result == [1.0, 2.0, 3.0]

        def test_cast_text_to_bool(self):
            result = scalify.cast("no", bool)
            assert result is False

        def test_cast_text_to_bool_with_true(self):
            result = scalify.cast("yes", bool)
    ...
    class TestPydantic:
        ...
        def test_pay_attention_to_field_descriptions(self, gpt_4):
            ...
            result = scalify.cast("I bought a Chevrolet", Car)
            assert result == Car(make="Ford")

    class TestInstructions:
        def test_cast_text_with_significant_instructions(self):
            result = scalify.cast("one", int, instructions="return the number 4")
            assert result == 4
        ...
        def test_cast_text_with_subtle_instructions(self, gpt_4):
            ...
            assert result == "My name is SCALIFY"
...
class TestMapping:
    def test_map(self):
        result = scalify.cast.map(["one", "two"], int)
        assert isinstance(result, list)
        assert result == [1, 2]

    def test_map_with_instructions(self):
        result = scalify.cast.map(
            ["one", "two"],
            int,
    ...
    async def test_async_map(self):
        result = await scalify.cast_async.map(["one", "two"], int)
        assert isinstance(result, list)
        assert result == [1, 2]
